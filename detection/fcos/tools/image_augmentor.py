...
def image_augmentor(image, input_shape, data_format, output_shape, zoom_size=None,
    ...
    :param ground_truth: [xmin, ymin, xmax, ymax, classid]
    :param pad_truth_to: pad ground_truth to size [pad_truth_to, 5] with -1
    :return image: output_shape
    :return ground_truth: [pad_truth_to, 5] [xmin, ymin, xmax, ymax, class_id]
    """
    if pad_truth_to is None or pad_truth_to < 1:
        raise Exception("pad_truth_to must > 0")
    ...
    if ground_truth is not None:
        ymin = tf.reshape(ground_truth[:, 1], [-1, 1])
        ymax = tf.reshape(ground_truth[:, 3], [-1, 1])
        xmin = tf.reshape(ground_truth[:, 0], [-1, 1])
        xmax = tf.reshape(ground_truth[:, 2], [-1, 1])
    ...
    if rotate is not None:
        ...
        if ground_truth is not None:
            ...
            xmin = tf.reduce_min(tf.concat([xminymin_x, xmaxymax_x, xminymax_x, xmaxymin_x], axis=-1), axis=-1,
                                 keepdims=True)
            ymin = tf.reduce_min(tf.concat([xminymin_y, xmaxymax_y, xminymax_y, xmaxymin_y], axis=-1), axis=-1,
                                 keepdims=True)
            xmax = tf.reduce_max(tf.concat([xminymin_x, xmaxymax_x, xminymax_x, xmaxymin_x], axis=-1), axis=-1,
                                 keepdims=True)
            ymax = tf.reduce_max(tf.concat([xminymin_y, xmaxymax_y, xminymax_y, xmaxymin_y], axis=-1), axis=-1,
                                 keepdims=True)
    if data_format == 'channels_first':
    ...
    if ground_truth is not None:
        ...
        h = ymax - ymin
        w = xmax - xmin
        ground_truth_ = tf.concat([x - w/2, y - h/2, x + w / 2, y + h/2, class_id], axis=-1)

        if tf.shape(ground_truth_)[0] == 0:
            if pad_truth_to is not None:
                ground_truth_ = tf.concat([xx-ww/2, yy-hh/2, xx+ww/2, yy+hh/2, class_id], axis=-1)
                ground_truth = tf.pad(
